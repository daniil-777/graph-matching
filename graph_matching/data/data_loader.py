...
from data.cub_voc import Cub_Voc
...
class GMDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        G1_gt, H1_gt, e1_gt = build_graphs(P1_gt, n1_gt, stg=cfg.PAIR.GT_GRAPH_CONSTRUCT, k = cfg.PAIR.NUM_NEIGHBOURS)
        if cfg.PAIR.REF_GRAPH_CONSTRUCT == 'same':
            G2_gt = perm_mat.transpose().dot(G1_gt)
            H2_gt = perm_mat.transpose().dot(H1_gt)
            e2_gt= e1_gt
        else:
            G2_gt, H2_gt, e2_gt = build_graphs(P2_gt, n2_gt, stg=cfg.PAIR.REF_GRAPH_CONSTRUCT, k = cfg.PAIR.NUM_NEIGHBOURS)
